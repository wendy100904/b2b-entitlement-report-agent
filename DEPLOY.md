...
This is suitable only for demos or a trusted internal network. Uploaded sessions are persisted to SQLite (`entitlement.db`), so they survive service restarts.
...
>
> 持久化说明：Render 免费实例的磁盘是临时的，容器重建后 SQLite 数据会清空。如需长期保留数据，请在 Render 上开通磁盘挂载，或设置 `DATABASE_URL` 指向外部 PostgreSQL（如 Render Postgres）。

### 快速自查（部署后）
- 打开 `https://xxx.onrender.com/api/v1/health` 应返回 `{"status":"ok",...}` 且包含 `"storage":"sqlite"`
- 打开首页 → 点「⬇ 下载样例数据」应能下载 `sample_data_multiweek.csv`
- 用样例上传 → 生成报告/周报 → 导出 PDF，确认全链路正常
- 重启服务后打开 `https://xxx.onrender.com/api/v1/sessions`，之前上传的会话仍在

## Production boundary

The current prototype has no login, tenant isolation, or audit logging. SQLite persistence keeps uploaded data across restarts, but for production rollout, point `DATABASE_URL` at a managed PostgreSQL, add SSO/authorization, object storage, and audit logging. Do not upload real company/customer data to a public URL until it is deployed inside a company network or extended with those controls.
